...
func (*VerificationToken) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case verificationtoken.FieldEmail, verificationtoken.FieldVerificationCode:
	...
	for i := range columns {
		...
		case verificationtoken.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				vt.user_verification_token = new(string)
				*vt.user_verification_token = value.String
			}
		default:
...
func (vt *VerificationToken) String() string {
	...
	builder.WriteString(vt.Email)
	builder.WriteString(", ")
	builder.WriteString("verification_code=")
	builder.WriteString(vt.VerificationCode)
	builder.WriteString(", ")
