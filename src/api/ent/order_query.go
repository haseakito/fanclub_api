...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if oq.withPost != nil || oq.withUser != nil {
		withFKs = true
	}
	if withFKs {
...
func (oq *OrderQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Order, init func(*Order), assign func(*Order, *Post)) error {
	...
	for i := range nodes {
		if nodes[i].post_orders == nil {
			continue
		}
		fk := *nodes[i].post_orders
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
