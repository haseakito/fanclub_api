...
// QueryBillboard chains the current query on the "billboard" edge.
func (aq *AssetQuery) QueryBillboard() *BillboardQuery {
	query := (&BillboardClient{config: aq.config}).Query()
...
func (aq *AssetQuery) Clone() *AssetQuery {
	...
	return &AssetQuery{
		...
		withBillboard: aq.withBillboard.Clone(),
...
// WithBillboard tells the query-builder to eager-load the nodes that are connected to
// the "billboard" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AssetQuery) WithBillboard(opts ...func(*BillboardQuery)) *AssetQuery {
	query := (&BillboardClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withBillboard = query
	return aq
}
...
func (aq *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [2]bool{
			aq.withBillboard != nil,
			aq.withPost != nil,
		}
	)
	if aq.withBillboard != nil || aq.withPost != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withBillboard; query != nil {
		if err := aq.loadBillboard(ctx, query, nodes, nil,
			func(n *Asset, e *Billboard) { n.Edges.Billboard = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AssetQuery) loadBillboard(ctx context.Context, query *BillboardQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Billboard)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Asset)
	for i := range nodes {
		if nodes[i].billboard_asset == nil {
			continue
		}
		fk := *nodes[i].billboard_asset
	...
	query.Where(billboard.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "billboard_asset" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AssetQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Post)) error {
	...
	nodeids := make(map[uuid.UUID][]*Asset)
	for i := range nodes {
		if nodes[i].post_assets == nil {
			continue
		}
		fk := *nodes[i].post_assets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_assets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
